...
; *****************************************************************
; AUTHORS: Jessica Olaniyi,  501236760
;          Tariye Inaibagha, 501169646
; *****************************************************************
...
LCD_E         EQU   $80                   ; LCD E-signal pin, PJ7 1000 0000
LCD_RS        EQU   $40                   ; LCD RS-signal pin, PJ6 0100 0000
...
; -------------------------------------------------------
THRESH_LINE     FCB   $9D        ;THRESHOLD VALUES FOR EACH SENSOR
THRESH_BOW      FCB   $CB
THRESH_MID      FCB   $CB
THRESH_PORT     FCB   $CC
THRESH_STBD     FCB   $CC
; Adding variance based on testing to act as a tolerance range/ zone
LINE_VAR          FCB   $18   ;E/F        
BOW_VAR           FCB   $30   ;A        
PORT_VAR          FCB   $40   ;B                  
MID_VAR           FCB   $20   ;C
STARBOARD_VAR     FCB   $15   ;D
;ASCII codes are written into these buffers then copied into the LCD
...
MAIN        
              ...
              JSR   G_LEDS_OFF             ; Disable the guider LEDs, to measure illuminated values only                   
              JSR   UPDT_DISPL              ;update battery volt and current EEBOT state
              LDAA  CRNT_STATE         
              JSR   DISPATCHER         
              BRA   MAIN               
...
VERIFY_STOP       CMPA  #STP                             ; Verify if the robot's state is ALL_STOP
                  BNE   VERIFY_LEFT_TRN                     ; If not, move to LEFT_TURN state validation
                  JSR   STP_ST                           ; Validate STP state
                  RTS                                         
...
NO_FWD_BUMP       BRSET   PORTAD0, $08, NO_FWD_REAR_BUMP      ; Checks if the stern bumper 
                  MOVB    #STP, CRNT_STATE               ; if true, enter the                   
...
STP_ST       BRSET   PORTAD0, %00000100, NO_START_BUMP                                       
...
;---------------------------------------------------------------------------
;                           Clear LCD Buffer
; This routine writes space and characters (ascii 20) into the LCD display
